...
class RobertaTokenizerSeparatePair(RobertaTokenizer):
    def __init__(self, vocab_file, merges_file, tokens_a_max_len=89, **kwargs):
        super().__init__(vocab_file, merges_file, **kwargs)
        self.tokens_a_max_len = tokens_a_max_len
    ...
    def build_inputs_with_special_tokens(
        ...
        cls = [self.cls_token_id]
        sep = [self.sep_token_id]
        pad = [self.pad_token_id] * (self.tokens_a_max_len - len(token_ids_0))
        return cls + token_ids_0 + sep + pad + sep + token_ids_1 + sep
    ...
    def get_special_tokens_mask(
        ...
        if token_ids_1 is None:
            return [1] + ([0] * len(token_ids_0)) + [1]
        return [1] + ([0] * len(token_ids_0)) + [1] + [0] * (self.tokens_a_max_len - len(token_ids_0)) + [1] + \
               ([0] * len(token_ids_1)) + [1]
    ...
    def create_token_type_ids_from_sequences(
        ...
        if token_ids_1 is None:
            return len(cls + token_ids_0 + sep) * [0]
        return (self.tokens_a_max_len + len(sep + token_ids_1 + sep)) * [0]
...
class RobertaTokenizerSeparatePairFast(RobertaTokenizerFast):
    def __init__(self, vocab_file, merges_file, tokens_a_max_len=89, **kwargs):
        super().__init__(vocab_file, merges_file, **kwargs)
        self.tokens_a_max_len = tokens_a_max_len
